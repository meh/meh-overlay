...
# 		SRC_URI="${SRC_URI}
# 			linguas_${X/-/_}? ( http://dev.gentoo.org/~armin76/dist/${MY_P}-xpi/${MY_P}-${X}.xpi )"
# 	fi
...
# 		if [ "${X}" != "en-US" ]; then
# 			SRC_URI="${SRC_URI}
# 				linguas_${X%%-*}? ( http://dev.gentoo.org/~armin76/dist/${MY_P}-xpi/${MY_P}-${X}.xpi )"
# 		fi
...
RDEPEND="java? ( virtual/jre )
	...
	xulrunner? ( >=net-libs/xulrunner-9999 )"
